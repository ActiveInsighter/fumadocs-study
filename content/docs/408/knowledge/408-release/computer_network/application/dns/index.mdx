---
title: "DNS"
description: "💡 低优先级 在选择题中偶尔有考查，需要掌握 DNS 的域名服务器的不同层级，以及域名解析过程的两种方式（迭代和递归）。 真题 3 选择题 10 40 16 40 20 40 DNS 使用 层次域名空间 来组织域名，将域名划分为多个级别，每"
source: "https://csgraduates.com/computer_network/application/dns/"
---
...
选择题
[10#40](/study_methods/408quiz/2010/#40)
[16#40](/study_methods/408quiz/2016/#40)
[20#40](/study_methods/408quiz/2020/#40)

### 层次域名空间

**DNS** 使用 **层次域名空间** 来组织域名，将域名划分为多个级别，每个级别之间以点（.）分隔。域名从左到右逐级递增，最右边是 **顶级域名**（TLD），然后是 **二级域名**， **三级域名**，以此类推。

例如， `www.example.com` 中， `.com` 是顶级域名， `example.com` 是二级域名， `www.example.com` 是子域名。

![DNS 图 1](/images/computer-network/computer-network-application-dns-001.svg)

**DNS** **层次域名空间** 的设计使得域名分布在不同的管理区域中，每个管理区域负责管理其自己的域名空间。这种分层结构有助于提高可扩展性和效率。

### 域名服务器

![DNS 图 2](/images/computer-network/computer-network-application-dns-002.svg)
...
![DNS 图 3](/images/computer-network/computer-network-application-dns-003.svg)
...
![DNS 图 4](/images/computer-network/computer-network-application-dns-004.svg)
...
![DNS 图 5](/images/computer-network/computer-network-application-dns-005.svg)
...
![DNS 图 6](/images/computer-network/computer-network-application-dns-006.svg)
