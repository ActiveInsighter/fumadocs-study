---
title: "链式表示"
description: "单链表、双链表、循环链表和静态链表的结构与操作。"
---
...
单链表的本质要求是：**结点通过指针建立逻辑次序，物理地址无需连续**。实际程序常用 `malloc`/`new` 动态申请结点，但也可以来自对象池、静态数组等其他存储管理方式。

<InlineSvg src="/images/data-structure/data-structure-linearlist-linked-002.svg" alt={"data next"} />

由于链式存储不要求整体连续空间，**单链表** 具有如下特点：
...
在这个例子中，`data` 的类型是 `int`，意味着这个 **链表** 用于存储整数。`data` 的类型可以按需要改为，例如 `float`、`char` 或者自定义的结构体类型，以存储不同类型的数据。
...
若要插入实际值 `value`，可通过 `Node *p = malloc(sizeof(Node)); p->data = value;` 来创建一个新 **结点**，然后再将新创建的 **结点** 通过以上函数插入。
...
```
```
// 在链表的第 pos 个数据位置插入（位置从 1 开始计数），返回值表示是否成功
bool insert_at_pos(Node *head, int pos, int value) {
    // 约定第一个数据结点的位置为 1，head 为头结点
