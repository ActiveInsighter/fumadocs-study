...
![/ dev home bin usr fd0 video0 ls cp · · · · · · · · · · · · · · · · · · · · · · ·](/images/operating-system/operating-system-files-dir-001.svg)
...
![路径解析过程：从 /home/alice/a.txt 找到文件内容 解析路径： / → home → alice → a.txt 每一步：读取“目录文件” → 在其中查找名字 → 取出 inode 号 → 读 inode ① 根目录 / inode 号 = 2 (约定) 目录文件 “/” . → inode 2 .. → inode 2 home/ → inode 18 etc/ → inode 19 inode #2 type=dir, perm=... block_ptr](/images/operating-system/operating-system-files-dir-002.svg)
...
![filename inode # A 1 B 3 C 4 D 1 E i 目录存储结构 inode # ref. cnt 1 2 2 0 3 1 4 1 5 0 inode 表 i 0 n 0](/images/operating-system/operating-system-files-dir-003.svg)
...
![filename inode # A 1 L 3 目录存储结构 inode # 数据块指针 # 1 300, 301 3 400 INode 表 n 文件块 "hello" .... /root/A 内容为文件 A 的路径](/images/operating-system/operating-system-files-dir-004.svg)
