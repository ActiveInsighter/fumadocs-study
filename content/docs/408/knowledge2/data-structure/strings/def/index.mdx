---
title: "定义和实现"
description: "串的定义、存储方式与基本操作。"
---

> **复习优先级：低。**
重点掌握串的基本概念、顺序存储与块链存储，并为后续模式匹配算法建立术语基础。

### 串的定义

**串（String）**是由零个或多个字符组成的有限序列。设字符集为 $\Sigma$，则串可记为

$$
S=a_1a_2\cdots a_n,\qquad a_i\in\Sigma.
$$

$n$ 称为串长；当 $n=0$ 时称为空串。需要区分**空串**与只包含空格字符的串：前者长度为 0，后者长度不为 0。
...
在**定长顺序存储表示**中，存储数组的最大容量预先固定，但串的**逻辑长度可以变化**，通常另设 `length` 字段记录当前长度。
