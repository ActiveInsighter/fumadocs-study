---
title: 每日温度
description: 用“右侧第一个更大元素”建立单调栈直觉，掌握索引入栈、出栈结算与线性复杂度分析。
---

# 每日温度

<Callout type="info" title="单调栈专题">
本节聚焦 **每日温度**。阅读时重点关注：栈中保存什么、保持什么单调性、何时弹栈，以及每个元素为何只会入栈和出栈至多一次。
</Callout>
...
```rust label="Rust"
impl Solution {
    /// 单调栈的本质是以“空间换时间”，记录之前已访问过的非递增子序列下标
