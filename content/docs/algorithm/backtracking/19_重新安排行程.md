...
这道题目还是很难的，之前我们用回溯法解决了如下问题：[组合问题](https://programmercarl.com/algo/backtracking/0077-combinations.html)，[分割问题](https://programmercarl.com/algo/backtracking/0093-restore-ip-addresses.html)，[子集问题](https://programmercarl.com/algo/backtracking/0078-subsets.html)，[排列问题](https://programmercarl.com/algo/backtracking/0046-permutations.html)。
...
如果对map 和 set 的实现机制不太了解，也不清楚为什么 map、multimap就是有序的同学，可以看这篇文章[关于哈希表，你该了解这些！](https://programmercarl.com/algo/hash-table/hash-table-basics.html)。
...
所以找到了这个叶子节点了直接返回，这个递归函数的返回值问题我们在讲解二叉树的系列的时候，在这篇[二叉树：递归函数究竟什么时候需要返回值，什么时候不要返回值？](https://programmercarl.com/algo/binary-tree/0112-path-sum.html)详细介绍过。
...
已经看习惯回溯法代码的同学，到叶子节点了习惯性的想要收集结果，但发现并不需要，本题的result相当于 [回溯算法：求组合总和！](https://programmercarl.com/algo/backtracking/0216-combination-sum-iii.html)中的path，也就是本题的result就是记录路径的（就一条），在如下单层搜索的逻辑中result就添加元素了。
